...
const MenWomen = () => {
    ...
    const getData = async () => {
        try {
            ...
            const { data } = await axios.get("products/category/men's%20clothing");
            setMenWomen(data);
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <h2 className='text-2xl font-bold mb-4 dark:text-white'>Men's Clothing</h2> {/* Heading */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"> {/* Responsive Grid */}
                {menWomen.slice(0, 6).map((item) => ( // Limit to the first 6 items
